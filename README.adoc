...
*Menu*
La clase menu fue creada para que haya una instancia previa a que el juegue comience, esta consta de dos botones y un cursor que muestra la opción que se quiera elegir.
Sus variables fueron declaradas con nombres muy confusos, se corrigieron para que se pueda entender mejor. 
Otra funcionalidad del menu, fue poder ser utilizado como boton de pausa dentro del juego. La dificultad que nos presentó y la cual no dimos con la solución, fue pedirle al menu que vuelva a comenzar el juego sin cerrar el entorno.
...
También, se puede incluir una descripción general del programa�c�mo
funciona.
...
La clase Menu tiene como estados: double posicionDelBotonJugarX, double posicionDelBotonJugarY, double posicionDelBotonSalirX, double posicionDelBotonSalirY, double posicionDelCursorX, double posicionDelCursorY, y dos Image, que son botonJugar y botonSalir.

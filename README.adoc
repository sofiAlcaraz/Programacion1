...
En general el metodo dibujarse de cada clase que lo requiera toma por parametro a un entorno de tipo Entorno,es void y 
solo dibuja la imagen que se indique en el constructor.
...
Conejo.java[]
----
La Clase Conejo tiene como estados:private double altura,double ancho,double x, double y,double velocidadDeSalto(movimiento vertical y
horizontal),double deslizarHaciaAbajo(deslizamiento hacia abajo),Image imagenConejoEsperando,Image imagenConejoIzquierda,
Image imagenConejoDerecha,String ultimoMovimiento.
...
hacia abajo.
-El metodo disparar es de tipo Rasengan y lo que hace es devolver un Rasengan con su "x" e "y"(posicion actual del conejo,a la hora de disparar).
-El metodo seFueDePantalla es de tipo boolean y lo que hace es verificar si el conejo se fue de la pantalla por la parte inferior ,es decir,
si su "y + altura / 2 < 600"  entonces el metodo devuelve true, caso contrario,false.
-El metodo chocasteAlgunAuto es de tipo boolean y toma por parametro un array de autos ,si el conejo coliciona con alguno de estos autos,devuelve true
sino false.
...
destruidos por algun rasengan.
-El metodo avanzar, de tipo void, lo que hace es que el auto avance para cierto lado segun su sentido,sumando a su x la velocidad o restando segun cual 
sea el caso, tambien esta constantemente aumentando su "y" sumandole la bajadaDePantalla(que es la velocidad con la que se desliza
hacia abajo el auto).Si su "y" es mayor a 600 entonces el auto reaparece en su posicion por defecto en la pantalla de arriba(la pantalla
que no se ve) y su su x es mayor o menor a cierto extremo segun su sentido el x  se resetea a su posicion por defecto en la pantalla
...
-El metodo deslizarseHaciaAbajo es de tipo void,toma por parametro a un entorno de tipo Entorno,es de tipo void y lo que hace 
...
la calle reaparece en la posicion por defecto ,en la pantalla de arriba(la que no se ve).
...
la pantalla.
-El metodo colisionasteConAuto es de tipo boolean y revisa y el rasengan colisiono con algun lado del auto pasado por parametro,Si colisiono
entonces devuelve true,caso contraio develve false.
...
*Constantes: 
*Sonidos:* sonidoSalto = "jump.wav", String sonidoRasengan = "rasengan.wav";
*Para creacion de autos:*final int altoDelAuto = 42,final int altoDeLaCalle = 220,int anchoDeAuto = 50,double velocidadDeBajadaDePantalla = 1;
*Objetos:
...
Cada array de callecon autos(callePrimaria o calleSecundaria) tiene un length ==16, en el for se recorre el array y agrega autos en cada posicion,

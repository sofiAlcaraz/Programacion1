...
Pablo Pagano <coso@algo.com>; Jose_Luis Villacorta <coso@algo.com>; Alcaraz Sofia <sofiialcaraz16@gmail.com>
 ...
 * Clase Auto: Cuenta con su método dibujar,avanzar(mover) y los getters de x , y, altura, ancho y velocidad.
...
también decidimos crear sólo dos calles con cuatro carriles cada una  (8 carriles en total) y un rasengan por pantalla.

Sólo se puede jugar con las teclas de flecha arriba ó `w`, y flechas laterales ó `a`, `d`, y el espacio para los disparos. 
...
Luego nos dimos cuenta de que el conejo no saltaba en medio de la calle, sino en cualquier lado,
para arreglar esto, cambiamos el metodo `saltar()`, para que salte bien así y se le resta `altoAuto / 2 + espacioEntreAutos`, de esta forma saltaria 
en la mitad, pero luego tuvimos que cambiar en que `y` creamos al conejo,para que comeinze saltando en una posicion razonable
lo que hicimos fue, al crear el conejo pasarle como parametro en su `y` = `(posicionPrimerAutoCallePrimaria + espacioEntreAutos + altoDelAuto / 2) * 3`,
de esta forma el conejo se posicionaria al comezar la calle y saltaria el espacio entre autos mas el alto/2. 
...
A la creamos le enseñamos a que se dibuje y deslice.
...
image::este Equipo/Documentos/crearAutos.png[]
...
Si colisiono entonces se crea un nuevo auto en la posicion `y` de cada auto por defecto ,y a la posicion `x` se le resta o suma segun su sentido 
el ancho de pantalla para que reaparezca en su posicion por defecto.
Si el rasengan colisiono con algun auto este se elimina ,el rasengan tambien y el puntaje sube y se agrega un nuevo auto en la posicion del arreglo
donde estaba el auto eliminado,con otra posicion en `x`para que la calle no se quede sin autos.
En caso de no colicionar el rasenga se dibuja y mueve ,y lo mismo con los autos.
...
- Como estado propio: altura,ancho, x,y,velocidadDeSalto(movimiento vertical y
...
- Como comportamiento: el conejo se sabe, `dibujar()`, hacerSonidoDeSalto,saltar,esperar,saltarIzquierda,saltarDerecha,disparar,seFueDePantalla, chocasteAlgunAuto.

 * El método `dibujar` es peculiar ya que dibuja la imagen según que movimiento hizo anteriormente, el `ultimoMovimiento` se indica en los métodos `saltar`(guarda en `ultimoMovimiento` el char ' '),`saltarIzquierda`(guarda en `ultimoMovimiento` el char 'i') y `saltarDerecha`(guarda en `ultimoMovimiento` el char 'd').
 ...
 * El método `disparar()` devuelve un `Rasengan` con su `x` e `y` correspondientes (la posición actual del conejo a la hora de disparar).
 
 * El método seFueDePantalla es de tipo boolean y lo que hace es verificar si el conejo se fue de la pantalla por la parte inferior ,es decir,
...
Creemos que el pensar mucho el nombre de las variables nos va a quedar para siempre y que
hay miles de formas de escribir el código. Además aprendimos que el orden en que se ejecuta el código es muy importante.
...
Aprendimos algo muy útil, que es el manejo de git, que además de todo lo que aprendimos de la materia, nos llevamos todos estos conocimientos
que van a ser útiles en el futuro y nos acercan a lo que es un verdadero proyecto en equipo.
...
esto es saber cuáles son las fortalezas y debilidades de los otros, de este modo, nuestros aportes al proyecto se van a acomodar de 
...
Durante el proceso de creación del juego el código sufrió muchos cambios, pero, para bien y esto se logró gracias a un buen trabajo en equipo
...
En conclusión, una simple taza de café, unos buenos mates, mucha práctica, el seguimiento del profesor, el espacio de consultas,
y un buen trabajo en equipo, logró que este trabajo funcione correctamente y
se termine a tiempo y a pesar de renegar con el `push` y `pull` en el git, lo logramos.

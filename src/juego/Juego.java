...
public class Juego extends InterfaceJuego {
	...
	public Juego() {
		...
		for (int i = 0; i < autosHaciaIzquierda.length; i++) {
			if (i < 4) {
				autosHaciaIzquierda[i] = new Auto(35, 50, posicionEnX, posicionAuto-distanciaEntreAuto-altoDelAuto, 2, false,	velocidadDeBajadaDePantalla);
				posicionEnX += autosHaciaIzquierda[i].getAncho() * 5;
			} else if (i < 8) {
				autosHaciaIzquierda[i] = new Auto(35, 50, posicionEnX, posicionAuto-distanciaEntreAuto-altoDelAuto*2, 1, false, velocidadDeBajadaDePantalla);
				posicionEnX += autosHaciaIzquierda[i].getAncho() * 5;
			} else if (i < 12) {
				autosHaciaIzquierda[i] = new Auto(35, 50, posicionEnX, (entorno.alto() / 10) * -4 + 70, 2, false, velocidadDeBajadaDePantalla);
				posicionEnX += autosHaciaIzquierda[i].getAncho() * 5;
			} else {
				autosHaciaIzquierda[i] = new Auto(35, 50, posicionEnX, (entorno.alto() / 10) * -4 - 70, 1, false, velocidadDeBajadaDePantalla);
				posicionEnX += autosHaciaIzquierda[i].getAncho() * 5;
			}
		}
